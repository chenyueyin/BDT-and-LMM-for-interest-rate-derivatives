...
dat1<-read_xlsx("Data Input.xlsx",sheet=1)
dat2<-read_xlsx("Data Input.xlsx",sheet=2)
...
####Value the Product
dat3<-read_xlsx("Data Input.xlsx",sheet=3)
...
Bermudan<-function() {
  ...
  X<-matrix(nrow = num_sim,ncol = 12)
    for (i in 1:12) {
      X[,i]=scale1(libor_matrix[(3+i),5,])
    }
  
  theta2=solve(t(X)%*%X+0.00001*diag(12))%*%t(X)%*%Y
